...
Site.find({}, (err, docs) => {
  ...
  docs.forEach((doc) => {
    ...
    const Unverified = mongoose.model(
      `${collectionPrefix}Unverified`,
      unverifiedSchema,
    ...
    const Comment = mongoose.model(`${collectionPrefix}Comment`, commentSchema);
    const Page = mongoose.model(pageRef, pageSchema);

    compiledModels[collectionPrefix] = {
      User,
      Unverified,
      Category,
      Tag,
