...
function addTagscontroller(req, res, next) {
  ...
  if (!_id) {
    // New tag
    model.findOne({ slug }, (err, duplicate) => {
      assert.ifError(err);
    ...
    model.findOne({ _id }, (err, doc) => {
      ...
      if (doc) {
        ...
        if (appDir === "categories") {
          const isParentUnique =
            !parent && !doc.parent ? false : parent != doc.parent;
          ...
          retrieveAncestors(model, parent, doc, res, (modified) => {
            modified.save((err) => {
              ...
              if (isParentUnique) {
                model.find({ ancestors: oldParent }, (err, docs) => {
                  assert.ifError(err);
...
function getTagsController(req, res, next) {
  const {
    originalUrl,
    query: { collectionPrefix, slug, _id },
  } = req;
...
function editTagsController(req, res, next) {
  const { currentUser, originalUrl } = req;
  const appDir = originalUrl.split("/").slice(-2, -1).toString();
  const { _id } = req.body;
...
function deleteTagsController(req, res, next) {
  const {
    originalUrl,
    currentUser,
    params: { _id },
  ...
  const appDir = _id
    ? originalUrl.split("/").slice(-2, -1).toString()
  ...
  if (_id) {
    ...
    model.findOne({ _id }).exec((err, doc) => {
      assert.ifError(err);
        ...
        if (appDir === "categories") {
          model.update(
            ...
            (err) => {
              ...
              model.find({ ancestors: _id }, (err, docs) => {
                assert.ifError(err);
    ...
    model.remove({ _id: { $in: ids } }, (err, response) => {
      assert.ifError(err);
      ...
      if (appDir === "categories") {
        model.update(
          ...
          (err) => {
            ...
            model.find({ ancestors: { $in: ids } }, (err, docs) => {
              assert.ifError(err);
