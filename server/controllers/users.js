...
let usersController = (req, res, next) => {
  ...
  if (collectionPrefix !== "" && !/^\w+$/.test(collectionPrefix)) {
    errors.collectionPrefix =
      "Only empty string or a combination of letters, numbers and an underscore are allowed!";
  } else if (site === undefined) {
    errors.collectionPrefix =
      "The site with the requested collectionPrefix didn't exist!";
  }

  if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(email))
    errors.email = "Invalid email address";
  if (!/^[A-Z0-9_-]+$/i.test(username)) errors.username = "Invalid username";
  if (password === "") errors.password = "Invalid password";
  if (code === "") errors.code = "Invalid verification code";
  if (!_.isEmpty(errors)) return res.status(401).json({ errors });

  if (validate) {
    // For asynchronous validation.

    compiledModels[collectionPrefix].User.findOne({ username }, (err, doc) => {
      assert.ifError(err);
    ...
    if (!code) {
      if (password) {
        ...
        compiledModels[collectionPrefix].User.findOne({ email }, (err, doc) => {
          assert.ifError(err);
          ...
          if (doc !== null) {
            ...
            bcrypt.compare(password, doc.hash, (err, isMatched) => {
              ...
              if (isMatched) {
                // Password and stored hash are matched.
                ...
                // Password and stored hash did not matched.
                res.status(401).json({
        ...
        compiledModels[collectionPrefix].User.findOne(query, (err, doc) => {
          assert.ifError(err);
          ...
          if (!isEmailUsed) {
            ...
            compiledModels[collectionPrefix].Unverified.updateOne(
              { email },
              {
                email,
                code: generatedCode,
      ...
      if (username === undefined && password === undefined) {
        // Confirm the requested verification code.

        compiledModels[collectionPrefix].Unverified.findOne(
          {
            email,
            code,
            expires: { $gt: Date.now() },
          ...
          (err, doc) => {
            ...
            res.status(doc === null ? 401 : 200).json({
              state: { email, isVerified: doc !== null },
              errors:
                doc === null
                  ? {
        ...
        // Remove from `Unverified` and register the account.
        ...
        compiledModels[collectionPrefix].Unverified.findOne(
          ...
          (err, unverifiedUser) => {
            ...
            if (unverifiedUser !== null) {
              compiledModels[collectionPrefix].User.findOne(
                { username },
                (err, existingUser) => {
                  assert.ifError(err);
                  ...
                  if (existingUser === null) {
                    bcrypt.genSalt(SALT_ROUNDS, function (err, salt) {
                      bcrypt.hash(password, salt, function (err, hash) {
                        ...
                        compiledModels[collectionPrefix].User.create(
                          {
